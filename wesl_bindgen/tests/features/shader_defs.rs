...
use syn::parse_str;
use wesl_bindgen::{assert_tokens_snapshot, *};
...
fn test_shader_defs_basic() -> Result<()> {
	...
	WgslBindgenOptionBuilder::default()
		...
		.entry_points(vec![
			"tests/shaders/features/shader_defs/test_shader.wgsl".to_string()
		])
...
fn test_shader_defs_with_texture() -> Result<()> {
	...
	let actual =
		read_to_string("tests/output/features/shader_defs_with_texture.actual.rs")
			.unwrap();
	let parsed_output = parse_str(&actual).unwrap();
	assert_tokens_snapshot!(parsed_output);
	assert_rust_compilation!(parsed_output);
