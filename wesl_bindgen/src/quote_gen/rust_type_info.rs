	...
	match &ty.inner {
		...
		naga::TypeInner::Vector { size, scalar } => {
			...
			if let Some(ty) = rust_type {
				...
				let (stride, _) =
					get_stride_and_padding(alignment, *size, scalar.width, options);
		...
		naga::TypeInner::Matrix {
			...
			let rust_type =
				map_naga_mat_type(*columns, *rows, *scalar, alignment, options);
		...
		naga::TypeInner::Array {
			...
			if options.serialization_strategy == WgslTypeSerializeStrategy::Bytemuck {
				...
				if element_size < actual_stride {
					...
					let padding_size_tokens =
						syn::parse_str::<TokenStream>(&padding_hex).unwrap();
