		...
		for entry in self.members.iter() {
			match entry {
				RustStructMemberEntry::Field(field) => {
					...
					mem_assignments
						.push(field.generate_member_instantiate(&init_var_name));
				}
				RustStructMemberEntry::Padding(padding) => {
			...
			.map(|entry| match entry {
				RustStructMemberEntry::Field(field) => {
					...
					let doc_comment = if self.is_directly_shareable() {
						...
						let doc =
							format!("offset: {offset}, size: {size}, type: `{ty_name}`");
				...
				RustStructMemberEntry::Padding(padding) => {
					padding.generate_member_definition()
				}
			})
			.collect::<Vec<_>>();
			...
			match entry {
				...
				RustStructMemberEntry::Padding(_) => {
					...
					if idx + 1 < self.members.len() {
						if let RustStructMemberEntry::Field(next_field) =
							&self.members[idx + 1]
						{
	...
	fn build_layout_assertion(
		...
		for m in &self.members {
			match m {
				RustStructMemberEntry::Field(field) => {
					let name = Ident::new(
						field.naga_member.name.as_ref().unwrap(),
						Span::call_site(),
					);
					...
					assert_member_offsets
						.push(quote!(assert!(#rust_offset == #expected_offset);));
