...
fn generate_shader_module_embedded(entry: &WgslEntryResult) -> TokenStream {
	...
	let create_shader_module_fn = format_ident!(
		"{}",
		WgslShaderSourceType::EmbedSource.create_shader_module_fn_name()
	);
...
impl<'a, 'b> ComposeShaderModuleBuilder<'a, 'b> {
	fn new(
		...
		let entry_source_path =
			entry.source_including_deps.source_file.file_path.as_path();

		// Convert Vec to FastIndexMap for consistent ordering
		let shader_defs_map: crate::FastIndexMap<
			String,
		...
		let dependency_modules = self
			...
			.map(|dep| {
				...
				let relative_file_path =
					get_path_relative_to(self.output_dir, &dep.file_path);
		...
		match self.source_type {
			...
			_ => {
				...
				let dependency_modules =
					self.build_shader_dependency_modules_statements();
				let relative_file_path =
