...
impl<'a> BindGroupEntriesStructBuilder<'a> {
	...
	fn create_entry_from_parameter(
		...
		let resource_type =
			self.get_resource_type_from_binding(&binding.binding_type.inner);
	...
	fn assign_entries_from_parameters(&self, param_var_name: Ident) -> Vec<TokenStream> {
		self.data
			...
			.map(|binding| {
				...
				let binding_name = Ident::new(&demangled_name.name, Span::call_site());
				let create_entry =
					self.create_entry_from_parameter(&param_var_name, binding);
		...
		naga::TypeInner::Image {
			...
			let view_dim = if *arrayed {
				match dim {
					naga::ImageDimension::D2 => {
						quote!(wgpu::TextureViewDimension::D2Array)
					}
              ...
              "1 Dimensional and 3 Dimensional textures have an array view dimension"
				...
				match dim {
					...
					naga::ImageDimension::Cube => {
						quote!(wgpu::TextureViewDimension::Cube)
					}
				}
			};
			...
			match class {
				naga::ImageClass::Sampled { kind, multi } => {
					let sample_type = match kind {
						...
						naga::ScalarKind::Float => {
							let filterable = check_texture_filterability(
								binding_name,
				...
				naga::ImageClass::Storage { format, access } => {
					...
					let format =
						syn::Ident::new(&format!("{format:?}"), Span::call_site());
