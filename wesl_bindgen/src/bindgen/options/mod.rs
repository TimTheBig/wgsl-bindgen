...
impl From<(&str, &str, TokenStream)> for OverrideStructFieldType {
	fn from(
		(struct_regex, field_regex, override_type): (&str, &str, TokenStream),
	) -> Self {
		Self {
			struct_regex: Regex::new(struct_regex)
				.expect("Failed to create struct regex"),
...
impl From<(&str, bool)> for OverrideTextureFilterability {
	fn from((binding_regex, filterable): (&str, bool)) -> Self {
		Self {
			binding_regex: Regex::new(binding_regex)
				.expect("Failed to create binding regex"),
			filterable,
		}
	}
}
