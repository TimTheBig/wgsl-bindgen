// File automatically generated by wesl_bindgen^
//
// ^ wesl_bindgen version 0.22.0
// Changes made to this file will not be saved.
// SourceHash: 685a19cebe067dd46294c4157a6a38ad91ae6b77eff5f94b2c3c4cfbeb254d74
	...
	pub fn create_shader_module_relative_path(
		...
		match self {
			Self::FullscreenEffects => {
				fullscreen_effects::create_shader_module_relative_path(
					device,
					base_dir,
					shader_defs,
					load_file,
			...
			Self::SimpleArrayDemo => {
				simple_array_demo::create_shader_module_relative_path(
					device,
					base_dir,
					shader_defs,
					load_file,
			...
			Self::GradientTriangle => {
				gradient_triangle::create_shader_module_relative_path(
					device,
					base_dir,
					shader_defs,
					load_file,
		...
		fn visit_dependencies_recursive(
			...
			for import in imports {
				...
				let import_source = match load_file(&full_import_path) {
					...
					Err(err) => {
						return Err(format!(
							"Failed to load import file {full_import_path}: {err}"
						));
					}
					Ok(content) => content,
				};
...
pub mod layout_asserts {
	...
	const FULLSCREEN_EFFECTS_PUSH_CONSTANTS_ASSERTS: () = {
		assert!(
			std::mem::offset_of!(fullscreen_effects::PushConstants, color_matrix) == 0
		);
	...
	const COMPUTE_DEMO_PARTICLE_PHYSICS_JOB_ASSERTS: () = {
		...
		assert!(
			std::mem::offset_of!(compute_demo::particle_physics::Job, direction) == 16
		);
	...
	const COMPUTE_DEMO_PARTICLE_PHYSICS_PARAMS_ASSERTS: () = {
		...
		assert!(
			std::mem::offset_of!(compute_demo::particle_physics::Params, damping) == 4
		);
					...
					wgpu::BindGroupLayoutEntry {
						...
						ty: wgpu::BindingType::Buffer {
							...
							min_binding_size: std::num::NonZeroU64::new(
								std::mem::size_of::<_root::global_bindings::GlobalUniforms>(
								) as _,
							),
						},
						count: None,
				...
				entries: &[
					...
					wgpu::BindGroupLayoutEntry {
						...
						ty: wgpu::BindingType::Buffer {
							...
							has_dynamic_offset: false,
							min_binding_size: std::num::NonZeroU64::new(
								std::mem::size_of::<_root::fullscreen_effects::Uniforms>()
									as _,
							),
						},
						count: None,
	...
	pub mod particle_physics {
		...
		impl WgpuBindGroup1 {
			pub const LAYOUT_DESCRIPTOR: wgpu::BindGroupLayoutDescriptor<'static> =
				wgpu::BindGroupLayoutDescriptor {
					label: Some(
						"ComputeDemoParticlePhysics::BindGroup1::LayoutDescriptor",
					),
					entries: &[
						#[doc = " @binding(0): \"jobs\""]
						wgpu::BindGroupLayoutEntry {
